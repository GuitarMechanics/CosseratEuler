...
% 종횡비 긴 경우(파트2 코드에 해당) 힘에 비례해서 댐핑올리기
% 마지막으로 진동때문에 뻗은 인덱스: force 2 length 5 rad 6
clear; clc;
% E = 200e9; %Pa<<sus304
...
for radindex = 9:length(r_values)
    ...
    for lengthindex = 1:length(lengthvalues)
        ...
        if l_r_ratio > 50 || l_r_ratio < 10
            fprintf(sprintf('Loop skipped by the l_r_ratio %0.2f\n',l_r_ratio))
            continue
        end
        if radindex <= 6 && lengthindex <= 6
            fprintf('Loop skipped: already calculated value\n');
            pause(0.05);
            continue
        end
            for forceindex = 1:length(force_REI_ratio)
                ...
                clc;
                SingleSectionCR_adaptivedamping(force,lengthval,resolution, timestepcount,r_values(radindex),E)
                clear global
                clf(1)
            end
        clf(2)
