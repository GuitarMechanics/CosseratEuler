function supportiveTypeCCR
    clc;
    clear all;
    hat=@(y)[0,-y(3),y(2);y(3),0,-y(1);-y(2),y(1),0];
    hat2=@(y)[y(1,1);y(1,3);y(3,3)];
    global p R j n m v u q w vs us vt ut qt wt vst ust vh uh vsh ush qh wh nLL mLL pt x y z p2 R2 n2 m2 v2 u2 q2 w2 vs2  us2 vst2 ust2 vt2 ut2 qt2 wt2 vh2 uh2 vsh2 ush2 qh2 wh2 nLL2 mLL2 pt2 xx yy zz Nc Mc%Make vars available in whole program
    ...
    rt11 = [0.01;0;0];                          %tendon offset- First arm
    rt12 = [0;0.01;0];                          %tendon offset- First arm
    rt21 = [0.01;0;0];                          %tendon offset- Second arm
    rt22 = [0;0.01;0];                          %tendon offset- Second arm
    rho = 6450;                                 %Density
    g = [-9.81;0;0];                            %Gravity
    ...
    dt = 0.02;                                  %Time step
    alpha = -0.25;                              %BDF-alpha parameter
    STEPS = 150;                                %Number of timesteps to completion
    vstar = @(s)[0;0;1];                        %Value of v when static and absent loading
    ustar = @(s)[0;1.62;0];                     %Precurvature
    vsstar = @(s)[0;0;0];
    usstar = @(s)[0;0;0];
    %Boundary Conditions
    for i = 1 : STEPS
    ...
    vstar2 = @(s)[0;0;1];                       %Value of v when static and absent loading
    ustar2 = @(s)[0;3.68;0];                    %Precurvature
    vsstar2 = @(s)[0;0;0];
    usstar2 = @(s)[0;0;0];
    %Boundary Conditions
    for i = 1 : STEPS
        p2{i,1} = [-0.22;0;-0.11];              %Clamped base
        R2{i,1} = eye(3);
        q2{i,1} = [0;0;0];
        w2{i,1} = [0;0;0];
    end
    ...
    ds2 = L2/(N2-1);                            %Grid distance (before strain)
    ...
    applyStaticBDFalpha();
    fsolve(@staticIVP2, zeros(6,1));            %Solve static BVP w/ shooting method
...
%         visualize2();
    ...
    function E = Errorfc(G)
        ...
        E = [(p{i,18} - p2{i,N2}-p{1,18} + p2{1,N2}) ; hat2(R2{i,N2}*R{i,18}' - R2{1,N2}*R{1,18}')];
    ...
    function E = dynamicIVP(G)
       ...
       for j = 18 : 18 
        [ps, Rs, ns, ms, qs, ws, vs{i,j}, us{i,j},...
            ...
            p{i,j+1} = p{i,j} + ds*ps;
            R{i,j+1} = R{i,j} + ds*Rs;
            n{i,j+1} = n{i,j} + Nc{i} + ds*ns;
            m{i,j+1} = m{i,j} + Mc{i} + ds*ms;
            q{i,j+1} = q{i,j} + ds*qs;
            w{i,j+1} = w{i,j} + ds*ws;
...
function E = dynamicIVP2(G)
        ...
        for j = 1 : N2-1
            [ps2, Rs2, ns2, ms2, qs2, ws2, vs2{i,j}, us2{i,j},...
                 ...
                 qt2{i,j}, wt2{i,j},vst2{i,j}, ust2{i,j}] = ...
                dynamicODE2(p2{i,j},R2{i,j},n2{i,j},m2{i,j},q2{i,j},w2{i,j});
            p2{i,j+1} = p2{i,j} + ds2*ps2;
            R2{i,j+1} = R2{i,j} + ds2*Rs2;
            n2{i,j+1} = n2{i,j} + ds2*ns2;
            m2{i,j+1} = m2{i,j} + ds2*ms2;
    ...
    function [ps, Rs, ns, ms, vs, us, v, u] = staticODE(p,R,n,m)
        ...
        ptsb = hat(u)*rt11+v;
        Tt11 = 0;
        At = -Tt11/norm(ptsb)^3*hat(ptsb)*hat(ptsb);
        B = hat(rt11)*At;
        Gt = -At*hat(rt11);
        H =-B*hat(rt11);
        a = At*(hat(u)*ptsb);
    ...
    function [ps2, Rs2, ns2, ms2, vs2, us2, v2, u2] = staticODE2(p2,R2,n2,m2)
        ...
        ptsb = hat(u2)*rt21+v2;
        Tt21 = 0;
        At = -Tt21/norm(ptsb)^3*hat(ptsb)*hat(ptsb);
        B = hat(rt21)*At;
        Gt = -At*hat(rt21);
        H =-B*hat(rt21);
        a = At*(hat(u2)*ptsb);
        ...
        c = Kbt*usstar2(ds2*(j-1))-hat(u2)*Kbt*(u2-ustar2(ds2*(j-1)))-hat(v2)*Kse*(v2-vstar(ds2*(j-1)))-b;
    ...
    function [ps2,Rs2,ns2,ms2,qs2,ws2,vs2, us2,v2,u2,vt2,ut2,qt2,wt2,vst2,ust2] = dynamicODE2(p2,R2,n2,m2,q2,w2)
    ...
    function visualize1()
        ...
        title(str);  xlabel('z (m)');  ylabel('x (m)');zlabel('y (m)');
...
for i = 1 : STEPS, U(i)=i*dt; Y1(i)=p{i,N}(1)-p{1,N}(1);Y2(i)=p{i,N}(3)-p{1,N}(3); end
   subplot(2,1,1);
   plot(U,Y1);
   ...
   saveas(gcf, '../results/SupportiveTypeCCR.png')
end
